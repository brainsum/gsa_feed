...
namespace Drupal\gsa_feed\Plugin\views\row;
 ...
 * @ViewsRow(
 *   id = "gsa_feeds_fields",
 *   title = @Translation("GSA Feed Fields"),
 *   help = @Translation("Display fields as GSA feed items"),
 *   theme = "views_view_row_gsa_rss",
 *   display_types = {"feed"}
 * )
 */
class GSAFeedsField extends RowPluginBase {
  ...
  public function validate() {
    ...
    $plugin = $this->displayHandler->getPlugin('style');
    if ($plugin->getPluginId() !== 'gsa_rss') {
      $errors[] = $this->t('Row plugin %row requires an <em>GSA RSS Feed</em> style plugin.', ['%row' => $this->definition['title']]);
    }
  ...
  public function render($row) {
    ...
    try {
      $item->link = $row->_entity->toUrl('canonical', ['absolute' => TRUE])
        ->toString(TRUE)
        ->getGeneratedUrl();
    }
    catch (\Exception $e) {
    ...
    if (empty($item->link)) {
      ...
      if ($linkField = $this->getField($row_index, 'field_url_link')) {
        $item->link = $linkField;
      }
      else {
    ...
    $item->guid = [
      'key' => 'guid',
      'value' => $item_guid . ' at ' . Url::fromUserInput('/')->setAbsolute()->toString(),
